package com.project.digitalLibrary.configurations;
...
import com.project.digitalLibrary.adaptor.UserService;
...
public class SecurityConfig {
	 ...
	 * 
	 * We can define our own customized filters @EnableWebSecurity. Multiple filters are implemented in
	 * chained order. Together they form a filter chain. One by one filters are
	 * applied
	 */
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpsecurity) throws Exception {
		return httpsecurity.csrf(customizer -> customizer.disable()) // is how to disable csrf
				.authorizeHttpRequests(request -> request.anyRequest().authenticated()) // directly authentication of any HTTP Methods
				.formLogin(Customizer.withDefaults()) // when login for UI Form Page
				.httpBasic(Customizer.withDefaults()) // when login for Rest API
				.build();
	}
...
//	}
	...
	//Below is the way of using username and password from the rows stored in database
	/*
	 ...
	 * } 
	 * catch (ResourceNotFoundException e) { throw new
